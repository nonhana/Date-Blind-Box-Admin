...
export function useDispatchMenu() {
  ...
  const stateSetOpenMenuKey = useCallback(
    (keys: string[]) => dispatch(setOpenKey(keys)),
    [dispatch]
  );
  const stateSetSelectMenuKey = useCallback(
    (keys: string[]) => dispatch(setSelectKey(keys)),
    [dispatch]
  );
  const stateFilterOpenMenuKey = useCallback(
    (key: string[]) => dispatch(filterOpenKey(key)),
    [dispatch]
  );
  const stateAddOpenedMenu = useCallback(
    (menuItem: OpenedMenu) => dispatch(addOpenedMenu(menuItem)),
    [dispatch]
  );
  ...
  const stateSetCurrentPath = useCallback(
    (path: string) => dispatch(setCurrentPath(path)),
    [dispatch]
  );
  return {
    stateSetOpenMenuKey,
